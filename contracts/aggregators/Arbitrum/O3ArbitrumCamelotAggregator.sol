...
import "./interfaces/IArbitrumCamelotPair.sol";
import "../../assets/interfaces/IWETH.sol";
import "./libs/ArbitrumCamelotLibrary.sol";
import "../../crossChain/interfaces/IWrapper.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract O3ArbitrumCamelotAggregator is Ownable {
    ...
    uint256 public aggregatorFeeStable = 4 * 10 ** 6;
    ...
    function getAggFeeRate(address[] calldata path) internal view returns (uint) {
        address tokenA = path[path.length-1];
        address tokenB = path[path.length-2];

        IArbitrumCamelotPair pair = IArbitrumCamelotPair(ArbitrumCamelotLibrary.pairFor(factory, tokenA, tokenB));
        if (pair.stableSwap()) {
    ...
    function swapExactPTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint feeAmount = amountOut * getAggFeeRate(path) / FEE_DENOMINATOR;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (pull) {
            ...
            IERC20(path[0]).safeTransfer(ArbitrumCamelotLibrary.pairFor(factory, path[0], path[1]), amountIn);
        }
