...
import "./libs/ArbitrumZyberLibrary.sol";
...
contract O3ArbitrumZyberAggregator is Ownable {
    ...
    address public factory = 0xaC2ee06A14c52570Ef3B9812Ed240BCe359772e7;
    address public O3Wrapper = 0x446eb3aC5e6267931eD1198203B12CAfcd2E6240;
    address public feeCollector;

    uint256 public aggregatorFee = 25 * 10 ** 6;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (pull) {
            IERC20(path[0]).safeTransferFrom(
                msg.sender, ArbitrumZyberLibrary.pairFor(factory, path[0], path[1]), amountIn
            );
        } else {
            IERC20(path[0]).safeTransfer(ArbitrumZyberLibrary.pairFor(factory, path[0], path[1]), amountIn);
        }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(ArbitrumZyberLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)) - balanceBefore;
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = ArbitrumZyberLibrary.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(ArbitrumZyberLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = ArbitrumZyberLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? ArbitrumZyberLibrary.pairFor(factory, output, path[i + 2]) : _to;
