...
import "./interfaces/IBNBChainNomiswapPair.sol";
import "./interfaces/IBNBChainNomiswapStablePair.sol";
import "../../assets/interfaces/IWETH.sol";
import "./libs/BNBChainNomiswapLibrary.sol";
import "../../crossChain/interfaces/IWrapper.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract O3BNBChainNomiswapAggregator is Ownable {
    ...
    address public factory = 0xd6715A8be3944ec72738F0BFDC739d48C3c29349;
    address public stableSwapFactory = 0xC6B7ee49D386bAe4FD501F2d2f8d18828F1f6285;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (pull) {
            IERC20(path[0]).safeTransferFrom(
                msg.sender, _getPair(path[0], path[1]), amountIn
            );
        } else {
            IERC20(path[0]).safeTransfer(_getPair(path[0], path[1]), amountIn);
        }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(_getPair(path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)) - balanceBefore;
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = BNBChainNomiswapLibrary.sortTokens(input, output);
            (address _pair, address _factory) = _getPairAndFactory(input, output);
            IBNBChainNomiswapPair pair = IBNBChainNomiswapPair(_pair);
            uint amountInput;
            uint amountOutput;
            ...
            if (_factory == stableSwapFactory) {
                amountOutput = IBNBChainNomiswapStablePair(_pair).getAmountOut(input, amountInput);
            } else {
                amountOutput = BNBChainNomiswapLibrary.getConstantProductAmountOut(amountInput, reserveInput, reserveOutput, pair.swapFee());
            }
            }
    ...
    function _getPairAndFactory(address tokenA, address tokenB) internal view returns (address, address) {
        ...
        address pair = IBNBChainNomiswapFactory(_factory).getPair(tokenA, tokenB);
        if (pair == address(0)) {
            _factory = factory;
            pair = BNBChainNomiswapLibrary.pairFor(_factory, tokenA, tokenB);
        }
        return (pair, _factory);
    }

    function _getPair(address tokenA, address tokenB) internal view returns (address pair) {
        pair = IBNBChainNomiswapFactory(stableSwapFactory).getPair(tokenA, tokenB);
        if (pair == address(0)) {
            pair = BNBChainNomiswapLibrary.pairFor(factory, tokenA, tokenB);
        }
    }
}
