...
import "./libs/MetisNetswapLibrary.sol";
...
contract O3MetisNetswapAggregator is Ownable {
    ...
    address public O3Wrapper = 0x6BBA09a54A9AC72Ce74cbeB8aFa93DaF94cA1CE3;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (pull) {
            ...
            IERC20(path[0]).safeTransfer(MetisNetswapLibrary.pairFor(factory, path[0], path[1]), amountIn);
        }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            require(INetswapFactory(factory).getPair(input, output) != address(0), "O3Aggregator: PAIR_NOT_EXIST");
