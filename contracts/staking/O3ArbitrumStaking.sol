...
contract O3ArbitrumStaking is Context, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_rewardToken != address(0), "O3Staking: ZERO_REWARDTOKEN_ADDRESS");
    ...
    function getTotalProfit(address staker) external view returns (uint) {
        ...
        if (_upBlockTimestamp < block.timestamp) {
            uint unitProfitIncrease = _unitProfit * (block.timestamp - _upBlockTimestamp);
            currentProfitAccumu = _unitProfitAccumu[_upBlockTimestamp] + unitProfitIncrease;
        }
    ...
    function stake(uint amount) external nonReentrant _logs_ {
        ...
        totalStaked = amount + totalStaked;
        _updateUnitProfitState();
    ...
    function claimProfit() external nonReentrant _logs_ {
        ...
        _pushRewardToken(_msgSender(), totalProfit);
    }
    ...
    function _updateUnitProfitState() internal {
        ...
        uint unitStakeProfitIncrease = _unitProfit * (currentBlockTimestamp - _upBlockTimestamp);
        _unitProfitAccumu[currentBlockTimestamp] = unitStakeProfitIncrease + _unitProfitAccumu[_upBlockTimestamp];
    ...
    function _updateUnitProfit() internal {
        if (totalStaked > 0) {
            _unitProfit = _sharePerSecond * ONE / totalStaked;
        }
    }
    ...
    function _pushRewardToken(address to, uint amount) internal {
